...
再次运行安装程序会安全更新：安装器能够识别并替换旧版 Codex Connection 启动器，但绝不会覆盖同名的无关文件。若选择不生成可选重启入口，只会清理本项目以前生成的重启入口。
...
仓库同时包含 [Codex 专用代理组件桥接工具](component-bridge/README.md)。它可以经检测到的进程级代理启动 Codex 本体，并通过同一路由启动用户明确审批的配套工具。

- Codex 未运行时，桥接工具会创建一个带代理的新 Codex 进程。
- Codex 已运行时，只会将现有窗口置于前台；代理不能追溯注入已经存在的进程。
- 每个配套程序都必须通过 Windows **是/否** 对话框审批，授权与其准确路径和 SHA-256 校验值绑定。
- 桥接工具只传递代理环境变量，不会复制 Codex Cookie、登录令牌、账号数据或会话。

连接器与组件桥接工具会作为同一个 GitHub Release 中的两个独立下载包提供，用户可以只安装需要的部分。
